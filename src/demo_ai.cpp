...
void demoVoiceCloning() {
    ...
    if (success) {
        ...
        std::cout << "✓ Model loaded: " << model.speakerEmbedding.size() << " features\n";
        ...
        std::cout << "✓ Voice converted with +" << settings.pitchShift << " semitones pitch shift\n";
    ...
    if (!voices.empty()) {
        ...
        auto speech = tts.synthesize("Welcome to MolinAntro DAW ACME Edition", voices[0], prosody);
        std::cout << "✓ Generated TTS: " << speech.getNumSamples() << " samples\n";
        std::cout << "  Duration: " << speech.getNumSamples() / 48000.0f << " seconds\n";
    }
    ...
    auto vocal = synth.synthesize(melody, "Do re mi fa sol",
                                  AI::VocalSynthesizer::VoiceStyle::Pop_Female, expr);

    std::cout << "✓ Vocal synthesized: " << vocal.getNumSamples() << " samples\n";
    std::cout << "  Duration: " << vocal.getNumSamples() / 48000.0f << " seconds\n";
...
void demoAIMastering() {
    ...
    std::cout << "✓ Pitch corrected with " << corrSettings.strength << "% strength\n";
    ...
    for (size_t i = 0; i < harmonies.size(); ++i) {
        std::cout << "  Harmony " << (i + 1) << ": " << harmonies[i].getRMSLevel(0) << " RMS\n";
    }
}
...
void demoMusicalAnalysis() {
    ...
    for (const auto& chord : chords) {
        std::cout << "  • " << chord.name << " at " << std::fixed << std::setprecision(2)
                  << chord.startTime << "s (confidence: "
                  << std::setprecision(1) << (chord.confidence * 100) << "%)\n";
    }
    ...
    std::cout << "  Confidence:   " << std::setprecision(1) << (key.confidence * 100) << "%\n";
    ...
    std::cout << "✓ Transcribed " << sequence.getNotes().size() << " MIDI notes\n";

    if (!sequence.getNotes().empty()) {
        std::cout << "\n  First 3 notes:\n";
        for (size_t i = 0; i < std::min(size_t(3), sequence.getNotes().size()); ++i) {
...
void demoGPUAcceleration() {
    ...
    std::cout << "  Total memory:   " << (deviceInfo.totalMemory / (1024 * 1024)) << " MB\n";
    std::cout << "  Free memory:    " << (deviceInfo.freeMemory / (1024 * 1024)) << " MB\n";
    std::cout << "  Float16 support: " << (deviceInfo.supportsFloat16 ? "Yes" : "No") << "\n";
    std::cout << "  Float64 support: " << (deviceInfo.supportsFloat64 ? "Yes" : "No") << "\n";
    ...
    gpu.resetStats();
    gpu.fastConvolve(signal.data(), kernel.data(), result.data(),
...
int main() {
    ...
    std::cout << "All AI features demonstrated successfully!\n\n";

    std::cout << "MolinAntro DAW ACME Edition is 100% ready with:\n";
    std::cout << "  ✓ Real-time voice cloning (RVC)\n";
    std::cout << "  ✓ Text-to-Speech synthesis\n";
    std::cout << "  ✓ AI vocal synthesis (14 styles)\n";
    std::cout << "  ✓ AI mastering (genre-aware, LUFS normalization)\n";
    std::cout << "  ✓ Neural pitch correction (formant-preserving)\n";
    std::cout << "  ✓ Neural harmony generation\n";
    std::cout << "  ✓ Chord detection & progression analysis\n";
    std::cout << "  ✓ Beat/tempo detection\n";
    std::cout << "  ✓ Key detection\n";
    std::cout << "  ✓ Audio-to-MIDI transcription\n";
    std::cout << "  ✓ GPU acceleration (CUDA/Metal/CPU)\n\n";
